...
module.exports = function (gulpWrapper, ctx) {
    ...
    gulp.task("__bump-version", function() {
        ...
        return gulp
            ...
            .pipe(shell(`${ctx.__config && ctx.__config.__npm ? ctx.__config.__npm : 'npm'} version ${targetVersion} --no-git-tag-version --scripts-prepend-node-path`, {cwd: ctx.baseDir, verbose: true}));
    });
    ...
    gulp.task("__publish", function() {
        ...
        return gulp
            .src("package.json", {cwd: ctx.baseDir})
            .pipe(shell(`${ctx.__config && ctx.__config.__npm ? ctx.__config.__npm : 'npm'} publish --tag=${tag} --git-tag-version=false --scripts-prepend-node-path`, {cwd: ctx.baseDir, verbose: true}));
    });
